...
public class PluginFinder : IPluginFinder
{
    ...
    /// <returns></returns>
    public IEnumerable<(TPlugin PluginInstance, string PluginId)> EnablePluginsFull<TPlugin>() where TPlugin : IPlugin // BasePlugin
    {
    ...
    /// <typeparam name="TPlugin">可以是一个接口，一个抽象类，一个普通实现类, 只要实现了 <see cref="IPlugin"/>即可</typeparam>
    /// <returns></returns>
    public IEnumerable<TPlugin> EnablePlugins<TPlugin>() where TPlugin : IPlugin // BasePlugin
    {
    ...
    /// </summary>
    /// <returns></returns>
    public IEnumerable<string> EnablePluginIds<TPlugin>() where TPlugin : IPlugin // BasePlugin
    {
        // 1.所有启用的插件 PluginId
        var pluginConfigModel = PluginConfigModelFactory.Create();
        IList<string> enablePluginIds = pluginConfigModel.EnabledPlugins;
